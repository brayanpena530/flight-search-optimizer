...
- `coverage.cachedAwards` / `candidates[].verification` — `discovered_cached`
  means the price must be reconfirmed with the airline.
...
needed; the compact packet is large already. `--pretty` for human reading,
default compact for machine parsing.
